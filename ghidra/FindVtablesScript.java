...
public class FindVtablesScript extends GhidraScript {
    ...
    public void run() throws Exception {
        ...
        var file = askFile("Save vtables", "Save");
        if (file == null || file.exists()) return;

        var writer = new PrintWriter(file, "UTF-8");
        var sortedVtables = new TreeMap<Long, String>(vtables);
        for (Map.Entry<Long, String> entry : sortedVtables.entrySet()) {
            long offset = entry.getKey();
            String tableName = entry.getValue();
            if (vtableOffsets.containsKey(offset) && vtableOffsets.get(offset) > 0) continue;
            writer.write(DemanglerUtil.demangle("_ZTV" + tableName).getNamespaceString().replace("::vtable", "") + " : 0x" + Long.toHexString(offset) + "\n");
        }
        writer.close();
    }
