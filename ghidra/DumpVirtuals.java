            ...
            while (isStartOfVtable(curAddr) && !this.monitor.isCancelled()) {
                ...
                while (!this.monitor.isCancelled()) {
                    ...
                    if (functionAddr.getUnsignedOffset() == 0) {
                        virtuals.add("empty_virtual_" + curAddr.toString() + "()");
                    ...
                    if (function == null) {
                        if (virtuals.size() > 0) {
                            while (virtuals.get(virtuals.size() - 1).contains("empty_virtual")) {
                    ...
                    if (function.getName().contains("pure_virtual")) {
                        ...
                        var funcSig = comment.replaceAll("^(non-virtual thunk to )?", "");
                ...
                if (hasVtableComment(curAddr) || hasVtableComment(curAddr.add(PTR_SIZE))) {
                    if (virtuals.size() > 0) {
                        while (virtuals.get(virtuals.size() - 1).contains("empty_virtual")) {
                            virtuals.remove(virtuals.size() - 1);
                        }
